...
type SpamParams struct {
	...
	MinAmount string `json:"min_amount"` // min uaeth amount
	MaxAmount string `json:"max_amount"` // max uaeth amount
...
func ReadAllocationsInput(cfg Config) Allocations {
	...
	if allocations.SpamDelegations == nil && len(allocations.Delegations) == 0 {
		log.Printf("no delegations specified. defaulting to equal distribution of %s uaeth\n", cfg.DefaultBaseAmount)
